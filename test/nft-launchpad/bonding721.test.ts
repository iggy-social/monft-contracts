// npx hardhat test test/nft-launchpad/bonding721.test.ts
...
describe("IggyLaunchpad721Bonding", function () {
  let ethers: any;
  let directoryContract: any;
  let launchpadContract: any;
  let statsContract: any;
  let statsMiddlewareContract: any;
  let metadataContract: any;
  let factoryOwner: any;
  let feeReceiver: any;
  let nftContractOwner: any;
  let user1: any;
  let user2: any;
  let referrer: any;
  let feePercent: any;
  let priceLaunch: any; // price for launching a new NFT contract
  let ratio: any; // ratio for the bonding curve
  ...
  before(async function () {
    ...
    feePercent = ethers.parseEther("0.05");
    priceLaunch = ethers.parseEther("69"); // price for launching a new NFT contract
    ratio = ethers.parseEther("6969"); // ratio for the bonding curve
  });
  ...
  beforeEach(async function () {
    ...
    await statsContract.setStatsWriterAddress(await statsMiddlewareContract.getAddress());

    const NftMetadata = await ethers.getContractFactory("NftMetadataMedia");
    ...
    await directoryContract.waitForDeployment();
    ...
    launchpadContract = await IggyLaunchpad721Bonding.deploy(
      ...
      feeReceiver.address,
      await directoryContract.getAddress(),
    ...
    await directoryContract.addWriter(await launchpadContract.getAddress());
    // add launchpad contract address to stats middleware contract as writer
    await statsMiddlewareContract.addWriter(await launchpadContract.getAddress());
  });
  ...
  it("creates a new NFT contract via launchpad and mints&burns a bunch of NFTs", async function () {
    ...
    console.log("user1BalanceBefore: " + ethers.formatEther(user1BalanceBefore) + " ETH");

    // get fee receiver balance before
    const feeReceiverBalanceBefore = await ethers.provider.getBalance(feeReceiver.address);
    console.log("feeReceiverBalanceBefore: " + ethers.formatEther(feeReceiverBalanceBefore) + " ETH");
    ...
    console.log("user1BalanceAfter: " + ethers.formatEther(user1BalanceAfter) + " ETH");

    // get fee receiver balance after
    const feeReceiverBalanceAfter = await ethers.provider.getBalance(feeReceiver.address);
    console.log("feeReceiverBalanceAfter: " + ethers.formatEther(feeReceiverBalanceAfter) + " ETH");
    ...
    console.log("Burn Price 1: " + ethers.formatEther(burnPrice1) + " ETH");

    // get mint price 1
    const mintPrice1 = await nftContract.getMintPrice();
    console.log("Mint Price 1: " + ethers.formatEther(mintPrice1) + " ETH");

    // mint 1 NFT
    const tx1 = await nftContract.connect(user2).mint(user2.address, { value: mintPrice1 });
    const receipt1 = await tx1.wait();
    //calculateGasCosts("mint1", receipt1);

    // get user1 balance 1
    const user1Balance1 = await ethers.provider.getBalance(user1.address);
    console.log("user1 balance 1: " + ethers.formatEther(user1Balance1) + " ETH");

    // get fee receiver balance 1
    const feeReceiverBalance1 = await ethers.provider.getBalance(feeReceiver.address);
    console.log("fee receiver balance 1: " + ethers.formatEther(feeReceiverBalance1) + " ETH");

    // get user2 balance 1
    const user2Balance1 = await ethers.provider.getBalance(user2.address);
    console.log("user2 balance 1: " + ethers.formatEther(user2Balance1) + " ETH");

    // get NFT contract balance 1
    const nftContractBalance1 = await ethers.provider.getBalance(nftContractAddress);
    console.log("NFT contract balance 1: " + ethers.formatEther(nftContractBalance1) + " ETH");
    ...
    console.log("Burn Price 2: " + ethers.formatEther(burnPrice2) + " ETH");

    // get mint price 2
    const mintPrice2 = await nftContract.getMintPrice();
    console.log("Mint Price 2: " + ethers.formatEther(mintPrice2) + " ETH");

    // revert if user1 tries to burn NFT #2 (user2 owns it)
    await expect(nftContract.connect(user1).burn(2)).to.be.revertedWith(
      "Nft721Bonding: caller is not owner nor approved"
    );

    // burn NFT #2
    const tx2 = await nftContract.connect(user2).burn(2); // token ID #2 (user2 owns it, while user1 owns token ID #1)
    const receipt2 = await tx2.wait();
    //calculateGasCosts("burn", receipt2);

    // get user1 balance 2
    const user1Balance2 = await ethers.provider.getBalance(user1.address);
    console.log("user1 balance 2: " + ethers.formatEther(user1Balance2) + " ETH");

    // get fee receiver balance 2
    const feeReceiverBalance2 = await ethers.provider.getBalance(feeReceiver.address);
    console.log("fee receiver balance 2: " + ethers.formatEther(feeReceiverBalance2) + " ETH");

    // get user2 balance 2
    const user2Balance2 = await ethers.provider.getBalance(user2.address);
    console.log("user2 balance 2: " + ethers.formatEther(user2Balance2) + " ETH");

    // get NFT contract balance 2
    const nftContractBalance2 = await ethers.provider.getBalance(nftContractAddress);
    console.log("NFT contract balance 2: " + ethers.formatEther(nftContractBalance2) + " ETH");
    ...
    console.log("Burn Price 3: " + ethers.formatEther(burnPrice3) + " ETH");

    // get mint price 3
    const mintPrice3 = await nftContract.getMintPrice();
    console.log("Mint Price 3: " + ethers.formatEther(mintPrice3) + " ETH");

    // mint NFT #3
    const tx3 = await nftContract.connect(user2).mint(user2.address, { value: mintPrice3 });
    const receipt3 = await tx3.wait();
    //calculateGasCosts("mint3", receipt3);

    // get user1 balance 3
    const user1Balance3 = await ethers.provider.getBalance(user1.address);
    console.log("user1 balance 3: " + ethers.formatEther(user1Balance3) + " ETH");

    // get fee receiver balance 3
    const feeReceiverBalance3 = await ethers.provider.getBalance(feeReceiver.address);
    console.log("fee receiver balance 3: " + ethers.formatEther(feeReceiverBalance3) + " ETH");

    // get user2 balance 3
    const user2Balance3 = await ethers.provider.getBalance(user2.address);
    console.log("user2 balance 3: " + ethers.formatEther(user2Balance3) + " ETH");

    // get NFT contract balance 3
    const nftContractBalance3 = await ethers.provider.getBalance(nftContractAddress);
    console.log("NFT contract balance 3: " + ethers.formatEther(nftContractBalance3) + " ETH");
    ...
    console.log("Burn Price 4: " + ethers.formatEther(burnPrice4) + " ETH");

    // get mint price 4
    const mintPrice4 = await nftContract.getMintPrice();
    console.log("Mint Price 4: " + ethers.formatEther(mintPrice4) + " ETH");

    // mint NFT #4
    const tx4 = await nftContract.connect(user2).mint(user2.address, { value: mintPrice4 });
    const receipt4 = await tx4.wait();
    //calculateGasCosts("mint4", receipt4);

    // get user1 balance 4
    const user1Balance4 = await ethers.provider.getBalance(user1.address);
    console.log("user1 balance 4: " + ethers.formatEther(user1Balance4) + " ETH");

    // get fee receiver balance 4
    const feeReceiverBalance4 = await ethers.provider.getBalance(feeReceiver.address);
    console.log("fee receiver balance 4: " + ethers.formatEther(feeReceiverBalance4) + " ETH");

    // get user2 balance 4
    const user2Balance4 = await ethers.provider.getBalance(user2.address);
    console.log("user2 balance 4: " + ethers.formatEther(user2Balance4) + " ETH");

    // get NFT contract balance 4
    const nftContractBalance4 = await ethers.provider.getBalance(nftContractAddress);
    console.log("NFT contract balance 4: " + ethers.formatEther(nftContractBalance4) + " ETH");
    ...
    console.log("Burn Price 5: " + ethers.formatEther(burnPrice5) + " ETH");

    // get mint price 5
    const mintPrice5 = await nftContract.getMintPrice();
    console.log("Mint Price 5: " + ethers.formatEther(mintPrice5) + " ETH");

    // mint NFT #5
    const tx5 = await nftContract.connect(user2).mint(user2.address, { value: mintPrice5 });
    const receipt5 = await tx5.wait();
    //calculateGasCosts("mint5", receipt5);

    // get user1 balance 5
    const user1Balance5 = await ethers.provider.getBalance(user1.address);
    console.log("user1 balance 5: " + ethers.formatEther(user1Balance5) + " ETH");

    // get fee receiver balance 5
    const feeReceiverBalance5 = await ethers.provider.getBalance(feeReceiver.address);
    console.log("fee receiver balance 5: " + ethers.formatEther(feeReceiverBalance5) + " ETH");

    // get user2 balance 5
    const user2Balance5 = await ethers.provider.getBalance(user2.address);
    console.log("user2 balance 5: " + ethers.formatEther(user2Balance5) + " ETH");

    // get NFT contract balance 5
    const nftContractBalance5 = await ethers.provider.getBalance(nftContractAddress);
    console.log("NFT contract balance 5: " + ethers.formatEther(nftContractBalance5) + " ETH");
    ...
    console.log("Burn Price 6: " + ethers.formatEther(burnPrice6) + " ETH");

    // get mint price 6
    const mintPrice6 = await nftContract.getMintPrice();
    console.log("Mint Price 6: " + ethers.formatEther(mintPrice6) + " ETH");

    // mint NFT #6
    const tx6 = await nftContract.connect(user2).mint(user2.address, { value: mintPrice6 });
    const receipt6 = await tx6.wait();
    //calculateGasCosts("mint6", receipt6);

    // get user1 balance 6
    const user1Balance6 = await ethers.provider.getBalance(user1.address);
    console.log("user1 balance 6: " + ethers.formatEther(user1Balance6) + " ETH");

    // get fee receiver balance 6
    const feeReceiverBalance6 = await ethers.provider.getBalance(feeReceiver.address);
    console.log("fee receiver balance 6: " + ethers.formatEther(feeReceiverBalance6) + " ETH");

    // get user2 balance 6
    const user2Balance6 = await ethers.provider.getBalance(user2.address);
    console.log("user2 balance 6: " + ethers.formatEther(user2Balance6) + " ETH");

    // get NFT contract balance 6
    const nftContractBalance6 = await ethers.provider.getBalance(nftContractAddress);
    console.log("NFT contract balance 6: " + ethers.formatEther(nftContractBalance6) + " ETH");
    ...
    console.log(
      "mintingFeePercentageBefore: " + Number(ethers.formatEther(mintingFeePercentageBefore)) * 100 + "%"
    );

    // fail to change minting fee via setMintingFeePercentage function in the NFT contract if new fee is too high (over 5%)
    await expect(
      nftContract.connect(user1).setMintingFeePercentage(ethers.parseEther("0.06"))
    ).to.be.revertedWith("Nft721Bonding: fee must be lower than 5%");

    // set minting fee to 1% via setMintingFeePercentage function in the NFT contract
    await nftContract.connect(user1).setMintingFeePercentage(ethers.parseEther("0.01"));

    // check minting fee after
    const mintingFeePercentageAfter = await nftContract.mintingFeePercentage();
    console.log("mintingFeePercentageAfter: " + Number(ethers.formatEther(mintingFeePercentageAfter)) * 100 + "%");

    expect(mintingFeePercentageAfter).to.equal(ethers.parseEther("0.01"));
  ...
  it("creates a few new NFT contracts via launchpad and queries them", async function () {
    ...
    console.log("referrerBalanceBefore: " + ethers.formatEther(referrerBalanceBefore) + " ETH");
    ...
    console.log("referrerBalanceAfter1: " + ethers.formatEther(referrerBalanceAfter1) + " ETH");
    ...
    console.log("referrerBalanceAfter2: " + ethers.formatEther(referrerBalanceAfter2) + " ETH");
    ...
    console.log("referrerBalanceAfter3: " + ethers.formatEther(referrerBalanceAfter3) + " ETH");
  ...
  it("get only mint price data", async function () {
    ...
    for (let i = 0; i < numberOfMintPriceData; i++) {
      ...
      console.log(ethers.formatEther(mintPrice));
      // mint NFT
      await nftContract.connect(user2).mint(user2.address, { value: mintPrice });
    }
  });
});
