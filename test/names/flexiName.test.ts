// npx hardhat test test/names/flexiName.test.ts
...
function calculateGasCosts(testName: string, receipt: any, ethers: any) {
  ...
  const gasCostMatic = ethers.formatUnits(
    String(Number(ethers.parseUnits("35", "gwei")) * Number(receipt.gasUsed)),
    "ether"
  );
  const gasCostEthereum = ethers.formatUnits(
    String(Number(ethers.parseUnits("21", "gwei")) * Number(receipt.gasUsed)),
    "ether"
  );
...
describe("FlexiPunkTLD", function () {
  ...
  it("should create a new valid domain", async function () {
    ...
    calculateGasCosts("Mint", receipt, ethers);

    // Check for DomainCreated event
    await expect(tx).to.emit(contract, "DomainCreated");

    const totalSupplyAfter = await contract.totalSupply();
    expect(totalSupplyAfter).to.equal(1n);

    // get referrer's balance AFTER
    const balanceReferrerAfter = await provider.getBalance(referrer.address);
    const referralFee = (domainPrice * 1000n) / 10000n; // 1000 bips = 10%
    expect(balanceReferrerAfter - balanceReferrerBefore).to.equal(referralFee);
    ...
    expect(firstDomainData.tokenId).to.equal(1n);
    ...
    expect(totalSupplyAfterSecond).to.equal(2n);
    ...
    expect(secondDomainData.tokenId).to.equal(2n);
    ...
    expect(totalSupplyAfterA).to.equal(3n);
    ...
    expect(aDomainData.tokenId).to.equal(3n);
    ...
    ).to.be.revertedWithCustomError(contract, "DomainNameEmpty");
  });
  ...
  it("should transfer domain to another user", async function () {
    ...
    const receipt = await tx.wait();
    calculateGasCosts("Transfer", receipt, ethers);
    ...
    expect(defaultNameAfterSigner).to.equal("");
  ...
  it("should change default domain", async function () {
    ...
    await expect(
      contract.connect(anotherUser).editDefaultDomain(
        newDomainName // trying to change back to techie (but msg.sender is not domain holder)
      )
    ).to.be.revertedWithCustomError(contract, "NotDomainHolder");
  });
  ...
  it("should change domain data", async function () {
    ...
    const receipt = await tx.wait();
    calculateGasCosts("DataChanged", receipt, ethers);

    await expect(tx).to.emit(contract, "DataChanged");
    ...
    await expect(
      contract.connect(anotherUser).editData(
        newDomainName, // domain name (without TLD)
        "No change"
      )
    ).to.be.revertedWithCustomError(contract, "NotDomainHolder");
  ...
  it("should change metadata", async function () {
    ...
    expect(domainData.tokenId).to.equal(1n);
    ...
    await metadataContract.changeDescription(await contract.getAddress(), newDesc);
    ...
    await expect(
      metadataContract.connect(anotherUser).changeDescription(await contract.getAddress(), newDesc)
    ).to.be.revertedWithCustomError(metadataContract, "SenderNotTldOwner");
  });
  ...
  it("should create a new valid domain, but with non-ascii letters input", async function () {
    ...
    const receipt = await tx.wait();
    calculateGasCosts("Mint " + newDomainName, receipt, ethers);
  ...
  it("should mint a token and burn it and mint it again", async function () {
    ...
    expect(totalSupplyBeforeMint).to.equal(0n);

    const balanceBeforeMint = await contract.balanceOf(signer.address);
    expect(balanceBeforeMint).to.equal(0n);
    ...
    expect(totalSupplyAfterMint).to.equal(1n);

    const balanceAfterMint = await contract.balanceOf(signer.address);
    expect(balanceAfterMint).to.equal(1n);
    ...
    expect(getDomainDataAfterMint.tokenId).to.equal(1n);
    ...
    const receipt = await tx.wait();
    calculateGasCosts("Burn domain", receipt, ethers);

    await expect(tx).to.emit(contract, "DomainBurned");

    const totalSupplyAfterBurn = await contract.totalSupply();
    expect(totalSupplyAfterBurn).to.equal(0n);
    ...
    expect(getDomainDataAfterBurn.tokenId).to.equal(0n);
    ...
    expect(totalSupplyAfterMintAgain).to.equal(1n);

    const balanceAfterMintAgain = await contract.balanceOf(signer.address);
    expect(balanceAfterMintAgain).to.equal(1n);
    ...
    expect(getDomainDataAfterMintAgain.tokenId).to.equal(2n); // token ID is now 2, because burned IDs still count as used
  ...
  it("should mint multiple tokens, burn one and mint it again", async function () {
    ...
    expect(idCounterBeforeMint).to.equal(1n);
    ...
    const totalSupplyAfterMint = await contract.totalSupply();
    expect(totalSupplyAfterMint).to.equal(3n);

    const idCounterAfterMint = await contract.idCounter();
    expect(idCounterAfterMint).to.equal(4n); // 3 token IDs has been created. The next domain will have ID 4.

    const balanceAfterMint = await contract.balanceOf(signer.address);
    expect(balanceAfterMint).to.equal(1n);

    const balanceAfterMint2 = await contract.balanceOf(anotherUser.address);
    expect(balanceAfterMint2).to.equal(1n);

    const balanceAfterMint3 = await contract.balanceOf(referrer.address);
    expect(balanceAfterMint3).to.equal(1n);
    ...
    expect(getDomainDataAfterMint2.tokenId).to.equal(2n);
    ...
    ).to.be.revertedWithCustomError(contract, "DomainAlreadyExists");

    // set domain data
    const domainDataString = "{'url': 'https://ethereum.org'}";
    await contract.connect(anotherUser).editData(newDomainName2, domainDataString);
    ...
    const receipt = await tx.wait();
    calculateGasCosts("Burn second domain", receipt, ethers);

    await expect(tx).to.emit(contract, "DomainBurned");

    const totalSupplyAfterBurn = await contract.totalSupply();
    expect(totalSupplyAfterBurn).to.equal(2n);

    const idCounterAfterBurn = await contract.idCounter();
    expect(idCounterAfterBurn).to.equal(4n);
    ...
    expect(getDomainDataAfterBurn.data).to.equal("");
    expect(getDomainDataAfterBurn.tokenId).to.equal(1n);
    ...
    expect(getDomainDataAfterBurn2.tokenId).to.equal(0n);
    ...
    expect(getDomainDataAfterBurn3.tokenId).to.equal(3n);
    ...
    expect(getDomainDataAfterMintAgain.tokenId).to.equal(4n); // token ID is now 4, because burned IDs still count as used
