...
/** 
@title Minter for .monft web3 names
@author Tempe Techie
*/
contract MONFTNameMinter is OwnableWithManagers, ReentrancyGuard {
  address public revenueAddress; // revenue revenue address address
  ...
  function mint(
    ...
    // send the rest to revenue address
    (bool sent, ) = revenueAddress.call{value: address(this).balance}("");
    require(sent, "Failed to send domain payment to revenue address");
  ...
  /// @notice This changes the revenue address in the minter contract
  function setRevenueAddress(address _revenueAddress) external onlyManagerOrOwner {
    revenueAddress = _revenueAddress;
  }
  ...
  function withdraw() external onlyManagerOrOwner {
    (bool success, ) = revenueAddress.call{value: address(this).balance}("");
